...
const uint8_t d_key[16] = {
	0x2b, 0x7e, 0x15, 0x27, 0x28, 0xae, 0xd2, 0x24, 0xab, 0xf7, 0x33, 0x26,
	0x09, 0xcf, 0x56, 0x3c
};

const uint8_t d_iv[16] = {
	0x6d, 0x01, 0x23, 0x03, 0x04, 0x15, 0x06, 0x66, 0x08, 0x79, 0x63, 0x08,
	0x0c, 0x0d, 0x32, 0x64
};
...
bool AESEncipher::aes_encrypt(uint8_t* &dest_buff, const uint8_t *src_buff, uint32_t& len)
{
    ...
    if(!tc_cbc_mode_encrypt(dest_bak, len+ENCRYPT_AES_IV_LEN, src_buff, len, (uint8_t*)_info.aesIv, &_info.aes_ctx)) {
		delete[] dest;
        delete[] dest_bak;
        delete[] src_buff;
	...
	memcpy(&dest[ENCRYPT_HEADER_LEN+sizeof(len)], &dest_bak[ENCRYPT_AES_IV_LEN], len);
...
bool AESEncipher::aes_decrypt(uint8_t* &dest_buff, const uint8_t *src_buff, uint32_t& len)
{
    ...
    if(!tc_cbc_mode_decrypt(dest, len+ENCRYPT_AES_IV_LEN, &src[ENCRYPT_AES_IV_LEN],  len+ENCRYPT_AES_IV_LEN, src, &_info.aes_ctx)) {
		delete[] dest;
        delete[] src;
